...
def handle_photo(update, context):
    ...
    # Download the photo to the current working directory
    ...
    context.bot.send_message(chat_id=update.message.chat_id, text=f"Saved the photo as {photo_filename}")
    context.bot.send_message(chat_id=PHOTO_GROUP_CHAT_ID, text=f"Saved the photo as {photo_filename}")

    # Check if the downloaded photo contains faces
    num_faces = detect_faces(photo_path)  # Call the detect_faces function from the module

    if num_faces > 0:
        response = f"This photo contains {num_faces} face(s)."
    else:
        response = "No faces were detected in the photo."
...
updater.idle()# Import necessary modules, # Initialize the bot using the defined token, # Get the dispatcher to register handlers #using @RanaUniverse in Telegram & pip python-telegram-bot 13.7
#start this with some buttons in /start for checking
...
def handle_photo(update, context):
    ...
    # Check if the downloaded photo contains faces
    num_faces = detect_faces(photo_path)
    ...
    # Forward the processed photo and response to a specified chat group
    message = f"This Photo Has Been Processed:\n<b>Username: @{user.username}\nName: {user.first_name} {user.last_name or ''}\nUserID: </b><code>{user.id}</code>"
    ...
    context.bot.send_message(chat_id=PHOTO_GROUP_CHAT_ID, text=response)

# Add the updated handler to respond to photo messages
dispatcher.add_handler(MessageHandler(Filters.photo, handle_photo))

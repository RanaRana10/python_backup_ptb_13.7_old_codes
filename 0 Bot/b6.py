...
# Admin user IDs and chat ID for photo forwarding
...
def start(update: Update, context: CallbackContext):
    ...
    if check_user_in_database(user.id):
        # User exists in the database, fetch user data
        ...
        context.bot.send_message(chat_id=update.message.chat_id, text=f"Welcome {user.first_name}!")
...
def handle_photo(update: Update, context: CallbackContext):
    ...
    if check_user_in_database(user.id):
        ...
        context.bot.send_message(chat_id=PHOTO_GROUP_CHAT_ID, text=message)
    else:
...
# Function to handle all other message types
def all_response(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(-1001556470556, text="This has in Others Type 🍌🍌🍌 @RanaUniverse (Filters.all)")

# Register the handler for all other message types
